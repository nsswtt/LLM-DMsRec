...
class SessionData(pl.LightningDataModule):
    ...
    def __init__(self, data_dir='../../data', name='beauty2014', validation=False, batch_size=100):
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None:
            self.train_data = pickle.load(open('../data/beauty2014/train.txt', 'rb'))
            if self.validation:
                self.train_data, self.valid_data = split_validation(self.train_data, 0.1)
            else:
                self.valid_data = pickle.load(open('../data/beauty2014/test.txt', 'rb'))
            self.train_data = Data(self.train_data, shuffle=True)
            self.valid_data = Data(self.valid_data, shuffle=True)


        if stage == 'test' or stage is None:
            self.test_data = pickle.load(open('../data/beauty2014/test.txt', 'rb'))
