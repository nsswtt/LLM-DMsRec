...
class MSGIFSR(nn.Module):
    ...
    def __init__(self, num_items, datasets, embedding_dim, num_layers, dropout=0.0, reducer='mean', order=3, norm=True, extra=True, fusion=True, device=th.device('cpu')):
        ...
        # for i in range(self.order):
        #     self.fc_sr.append(nn.Linear(input_dim*2, embedding_dim, bias=False))
        ...
        self.w = nn.Parameter(th.Tensor(embedding_dim, 768))
        self.b = nn.Parameter(th.Tensor(embedding_dim))
    ...
    def KLAlignmentModel(self, hidden1, hidden2):
        ...
        p_hidden1 = F1.softmax(hidden1, dim=-1) + 1e-8
        p_hidden2 = F1.softmax(hidden2, dim=-1) + 1e-8
        kl_loss = F1.kl_div(p_hidden2.log(), p_hidden1, reduction='batchmean')
        # kl_loss = F.kl_div(torch.log(p_hidden2), p_hidden1, reduction='batchmean')
        return kl_loss
        
    def forward(self, mg, explicit_responses, latent_responses):
        # explicit_responses = F1.dropout(explicit_responses, 0.5, training=self.training)
        # latent_responses = F1.dropout(latent_responses, 0.5, training=self.training)
        explicit_responses = th.matmul(explicit_responses, self.w.T) + self.b
        latent_responses = th.matmul(latent_responses, self.w.T) + self.b
        explicit_responses = explicit_responses.unsqueeze(1)
        latent_responses = latent_responses.unsqueeze(1)
        ...
        con_loss = self.KLAlignmentModel(sr_g.squeeze(1), explicit_responses.squeeze(1)) + self.KLAlignmentModel(sr_l.squeeze(1), latent_responses.squeeze(1))
        sr = th.cat([sr_l, sr_g], dim=-1)
        # sr   = th.cat([self.fc_sr[i](sr).unsqueeze(1) for i, sr in enumerate(th.unbind(sr, dim=1))], dim=1)
        # con_loss = self.KLAlignmentModel(sr.squeeze(1), explicit_responses.squeeze(1)) + self.KLAlignmentModel(sr.squeeze(1), latent_responses.squeeze(1))
        sr = self.linear_transform(sr)
        if self.norm:
