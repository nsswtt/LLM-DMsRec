...
def collate_fn_factory_ccs(seq_to_graph_fns, order):
    def collate_fn(samples):
        seqs, labels, explicit, latent = zip(*samples)
        ...
        labels = th.LongTensor(labels)
        explicit = th.FloatTensor(np.asarray(explicit))
        latent = th.FloatTensor(np.asarray(latent))

        return inputs, labels, explicit, latent
