...
def translation(data, item_dic):
    ...
    return (datax, datay, explicit, latent)
    # return (datax, datay)
...
class Data():
    ...
    def generate_batch(self, batch_size, shuffle = False):
        if shuffle:
            ...
            self.explicit_responses = self.explicit_responses[shuffled_arg]
            self.latent_responses = self.latent_responses[shuffled_arg]
    ...
    def get_slice(self, iList):
        inputs, targets, explicit_responses, latent_responses = self.inputs[iList], self.targets[iList], self.explicit_responses[iList], self.latent_responses[iList]
        ...
        return alias_inputs, H, HT, G, EG, items, targets, node_masks, edge_mask, edge_inputs, explicit_responses, latent_responses
